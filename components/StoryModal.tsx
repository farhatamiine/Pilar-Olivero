...
const StoryModal: React.FC<StoryModalProps> = ({ project, onClose }) => {
    ...
    return (
        <div
            id="modal-container"
            className="fixed inset-0 z-[100] bg-[#FAF9F6] overflow-y-auto scroll-smooth animate-in fade-in duration-1000 selection:bg-black selection:text-white"
        >
            <div className="fixed top-0 left-0 w-full z-[110] px-4 md:px-12 py-6 md:py-10 flex justify-between items-center mix-blend-difference text-white">
                <div className="flex items-center gap-6">
                    ...
                    <h2 className="font-serif text-base md:text-xl truncate max-w-[150px] sm:max-w-none">{project.title}</h2>
                </div>
                <button onClick={onClose} className="p-2 md:p-3 bg-white/10 backdrop-blur-md rounded-full hover:bg-white hover:text-black transition-all">
                ...
                <div className="max-w-4xl w-full grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-24 items-start">
                    <div className="lg:col-span-7">
                        ...
                        <div className="space-y-6 md:space-y-8 text-base md:text-lg text-neutral-600 font-light leading-relaxed">
                            ...
                            {project.conceptNote && <p className="italic border-l-2 border-neutral-100 pl-6 md:pl-8">{project.conceptNote}</p>}
                        </div>
                    </div>
                    <div className="lg:col-span-5 bg-[#f4f3f0] p-8 md:p-12 space-y-6 md:space-y-8 border border-neutral-100">
                        ...
                        <div className="space-y-3 md:space-y-4 font-mono text-[10px] md:text-xs text-neutral-500">
                            ...
                            <div className="flex justify-between border-b border-neutral-200 pb-2">
                                <span>TECHNICAL</span>
                                <span className="text-neutral-900 uppercase truncate ml-4">{project.technicalData}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <footer className="bg-neutral-900 py-24 md:py-32 flex flex-col items-center justify-center text-white text-center">
                ...
                <button onClick={onClose} className="font-serif text-xl md:text-2xl italic hover:opacity-50 transition-opacity flex items-center gap-4">
